...
extern "C" int _except_handler3 (_EXCEPTION_RECORD *pExceptionRecord, EXCEPTION_REGISTRATION_SEH3 *pRegistrationFrame, _CONTEXT *pContextRecord, EXCEPTION_REGISTRATION **pDispatcherContext)
{
    ...
    if (pExceptionRecord->ExceptionFlags & (EXCEPTION_UNWINDING | EXCEPTION_EXIT_UNWIND)) {
        ...
        asm volatile (
            ...
            "addl $8, %%esp;"
            "popl %%ebp;" : : "r"(scopeEbp), "r"(pRegistrationFrame), "i"(TRYLEVEL_NONE) : "eax", "ecx", "edx");
        ...
        if (filterFunclet) {
            ...
            if (filterResult != EXCEPTION_CONTINUE_SEARCH) {
                ...
                asm volatile (
                    ...
                    "jmp *%%ebx;" : : "r"(scopeEbp), "a"(currentTrylevel), "b"(handlerFunclet), "c"(newTrylevel), "d"(pRegistrationFrame) : "memory");
            }
        }
...
{
    DbgPrint("Throwing object at %x\n", pExceptionObject);
    DbgPrint("throw info at %x\n", pThrowInfo);

    EXCEPTION_RECORD exceptionRecord;
    exceptionRecord.ExceptionCode = 0xE06D7363; // FIXME: Magic number for C++ exceptions
