 ...
 *
 * @param channel The channel for which to register the handler function.
 * @param log_func The log function that'll be registered.
 */
uint8_t nxLogRegisterFunction(int channel, nxLogFunc log_func);

/**
 * Unregisters a log function.
 * This doesn't require the given function to be registered first.
 *
 * @param channel The channel for which to unregister the handler function.
 * @param log_func The log function that'll be unregistered.
 */
void nxLogUnregisterFunction(int channel, nxLogFunc log_func);

/**
 * Prints the raw message on screen.
 *
 * @param channel The channel in which to put the message.
 ...
 */
size_t nxLogPrint(int channel, const char *message);
 ...
 */
size_t nxLogPrintf(int channel, const char *format, ...) __attribute__((format(printf, 2, 3)));
