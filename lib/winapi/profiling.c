...
static uint64_t get_cpu_multiplier() {
    // Read EBL_CR_POWERON
    const uint64_t poweron_reg = __readmsr(0x2A);

    // TODO: Might not be a good idea, mainly done for xemu atm
    if (!poweron_reg) return 0;
...
static __cdecl VOID tsc_freq_init (VOID)
{
    ...
    if (multiplier_10x == 0) {
        DbgPrint("WARNING: Could not determine clock multiplier\n");
        return;
    }
...
__attribute__((section(".CRT$XXT"))) void (__cdecl *const __tsc_freq_init_p)(void) = tsc_freq_init;
