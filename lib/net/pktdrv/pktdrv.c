...
static void PktdrvMiiInterrupt(int mode)
{
    ...
    if ((mode == 0) && (state == g_s->OldPhyState)) {
        return; // All is ok, remain silent
    }
...
#endif
    if (mode == 0) {
        PktdrvStopSendRecv(); // Pktdrv is running. Stop it.
    }
    ...
    if (mode == 0) {
        PktdrvStartSendRecv(); // Mode changed. Restart Pktdrv.
...
static void __stdcall MyPktdrvDpc(PKDPC Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2)
{
    ...
    mii_status = 0;
    irq_status = NVREG_IRQSTAT_BIT0EVENT | NVREG_IRQSTAT_BIT1EVENT | NVREG_IRQSTAT_BIT2EVENT | NVREG_IRQSTAT_UNKEVENT;

    while (irq_status) {
        if (irq_status & NVREG_IRQSTAT_MIIEVENT) {
            PktdrvMiiInterrupt(0);
        }
...
int Pktdrv_Init(void)
{
    ...
    KeInitializeInterrupt(&s_MyInterruptObject, &MyPktdrvIsr, &g_s->MyContext, g_s->Vector, g_s->IrqLevel, LevelSensitive, TRUE);
